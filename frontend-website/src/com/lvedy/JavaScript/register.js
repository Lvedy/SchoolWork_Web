...
function handleRegisterSubmit(event) {
    ...
    if (password !== confirmPassword) {
        showMessage('密码和确认密码不匹配！', false);
        return;
    }
    
    if (password.length < 6) {
        showMessage('密码长度至少6位', false);
        return;
    }
    ...
    registerUser(username, password)
        .then(response => {
            if (response.success && response.code === 1) {
                showMessage('注册成功！欢迎加入全网最猛贩猪人大家庭！', true);
                // 清空表单
                ...
                showMessage(response.msg || '注册失败，请重试', false);
            }
        })
        .catch(error => {
            console.error('注册失败:', error);
            showMessage('注册失败，请检查网络连接或稍后重试', false);
        })
        .finally(() => {
            // 恢复提交按钮
